...
def validate_binary(path: Path) -> bool:
    """Basic sanity check that a downloaded sidecar is a real, non-truncated executable.

    A truncated download often still has a valid MZ/ELF header but fails at runtime
    with OS error 193 ("not a valid Win32 application"). We catch the cheap cases
    here: minimum size and the correct file magic. Returns True if plausible.
    """
    if not path.exists():
        return False
    try:
    ...
    return magic.startswith(b"\x7fELF") or magic.startswith((b"\xfe\xed\xfa", b"\xcf\xfa\xed\xfe"))
...
def download_sona(script_root: Path, target_triple: str | None) -> None:
    ...
    if sona_dest.exists() and (ffmpeg_dest is None or ffmpeg_dest.exists()):
        if validate_binary(sona_dest) and (ffmpeg_dest is None or validate_binary(ffmpeg_dest)):
            ...
            return
        print(f"Existing sidecar at {sona_dest} is invalid (truncated/corrupted); re-downloading.")
        for stale in (sona_dest, ffmpeg_dest):
    ...
    if not validate_binary(sona_dest) or (ffmpeg_dest is not None and not validate_binary(ffmpeg_dest)):
        print(f"ERROR: extracted sidecar is invalid (truncated/corrupted): {sona_dest}")
        print("The download may have been interrupted. Re-run this script to retry, or")
        print(f"place a valid binary manually at {sona_dest}.")
        raise SystemExit(1)
