		...
		<div>
			...
			{imageURL && (
				<div className={cn('flex', alignMap[align] || 'justify-start')}>
					<img src={`/audire${imageURL}`} alt={m.image()} className="rounded-lg transition-transform duration-500 ease-in-out hover:z-10 hover:scale-105" />
				</div>
			)}

			{videoURL && (
				<div className={cn('flex items-center', alignMap[align] || 'justify-start')}>
					<video src={`/audire${videoURL}`} controls className="rounded-lg" />
				</div>
			)}
		</div>
	)
