...
export function TuningSection({ vm }: { vm: SettingsViewModel }) {
	const pipeline = vm.preference.modelPipeline
								...
								<SectionCard>
									<div className="space-y-4">
										<div className="flex items-center justify-between">
											...
											<Switch checked={vm.preference.diarizeEnabled} onCheckedChange={vm.toggleDiarization} disabled={!pipeline.supportsDiarization} />
										</div>
										...
										{!pipeline.supportsDiarization && (
											<p className="text-sm text-muted-foreground">Diariazation not supported by {pipeline.engine} engine</p>
										)}
								...
								<SectionCard>
									<div className="space-y-4">
										<Field
											...
											<Select
												value={vm.preference.modelOptions.translate}
												onValueChange={(value) =>
													vm.preference.setModelOptions({ ...vm.preference.modelOptions, translate: value as 'none' | 'en' | 'zh' | 'ru' | 'ja' | 'fr' | 'de' | 'es' | 'ko' | 'it' })
												}
												disabled={!pipeline.supportsTranslation}
											>
										...
										{vm.preference.modelOptions.translate !== 'none' && (
											<p className="text-sm italic text-muted-foreground">{m.translateWhisperModelNote()}</p>
										)}
										{!pipeline.supportsTranslation && (
											<p className="text-sm text-muted-foreground">Translation not supported by {pipeline.engine} engine</p>
										)}
										...
										<div className="flex items-center justify-between">
											...
											<Switch
												...
												disabled={!pipeline.supportsWordTimestamps}
											/>
										</div>
										{!pipeline.supportsWordTimestamps && (
											<p className="text-sm text-muted-foreground">Word timestamps not supported by {pipeline.engine} engine</p>
										)}
