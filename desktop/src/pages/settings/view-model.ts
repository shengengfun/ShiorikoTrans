...
import { isTranscriptionModelFile, findModelFilesInDir, type ModelMetadata } from '~/lib/model'
...
export function viewModel() {
	...
	const [modelFiles, setModelFiles] = useState<NamedPath[]>([])
		...
		for (const entry of entries) {
			if (entry.is_dir) {
				...
				groups.push({
					...
					path: entry.path,
					is_dir: false,
					files: [{ name: entry.name, path: entry.path }],
				})
			}
		}
		setModelGroups(groups)

		const allFiles: NamedPath[] = []
		for (const g of groups) {
			for (const f of g.files) {
				allFiles.push({ name: f.name, path: f.path, is_dir: g.is_dir })
			}
		}
		setModels(allFiles)

		if (preference.modelPath) {
			const owningGroup = groups.find((g) => g.files.some((f) => f.path === preference.modelPath))
			if (owningGroup) {
				setSelectedGroupPath(owningGroup.path)
				setModelFiles(owningGroup.files)
			} else {
	...
	async function selectModelGroup(groupPath: string) {
		const group = modelGroups.find((g) => g.path === groupPath)
		if (!group) return
		setSelectedGroupPath(groupPath)
		setModelFiles(group.files)
		if (group.files.length === 1) {
			await selectModel(group.files[0].path)
		}
	}
	...
	async function deleteModel(modelPath: string) {
		try {
			...
			if (preference.modelPath === modelPath) {
				preference.setModelPath(null)
			}
			await loadModels()
	...
	async function getDefaultModel() {
		if (!preference.modelPath) {
			...
			if (modelGroups.length > 0) {
				...
				setSelectedGroupPath(firstGroup.path)
				setModelFiles(firstGroup.files)
				if (firstGroup.files.length > 0) {
					preference.setModelPath(firstGroup.files[0].path)
				}
			}
		}
	}
