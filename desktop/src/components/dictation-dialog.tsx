	...
	return (
		<Dialog>
			...
			<DialogContent className="max-w-md rounded-2xl border-border/60 bg-card/95 p-6 shadow-xl">
				<DialogHeader>
					<DialogTitle className="text-lg font-semibold">{m.globalDictation()}</DialogTitle>
				</DialogHeader>
						...
						<>
							<div className="space-y-1.5">
								<div className="flex items-center gap-2">
									<span className="text-xs text-muted-foreground">{m.globalHotkeyShortcut()}:</span>
							...
							<div className="flex gap-2">
								{(['clipboard', 'type'] as HotkeyOutputMode[]).map((mode) => (
									<button
										...
										{mode === 'clipboard' ? m.hotkeyOutputClipboard() : m.hotkeyOutputType()}
									</button>
								))}
							</div>
							<p className="text-[11px] italic text-muted-foreground/70">{m.globalHotkeyDescription()}</p>
						</>
					)}

					{!hotkey.hotkeyEnabled && (
						<p className="text-xs text-muted-foreground">{m.globalDictationPromo()}</p>
					)}
				</div>
			</DialogContent>
		</Dialog>
